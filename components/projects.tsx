...
export default function Projects() {
  ...
  const projects = [
    {
      title: 'Yukti Herbs',
      description: 'Premium herbal products e-commerce platform with seamless shopping experience. Focus on conversion optimization and user delight.',
      technologies: ['React', 'TypeScript', 'Tailwind CSS', 'Stripe'],
      image: '/premium-herbal-ecommerce-website-modern-design.jpg',
      link: '#',
      github: '#',
      featured: true,
    ...
    {
      title: 'Uniqaya',
      description: 'Fashion marketplace connecting independent designers with customers globally. Built with focus on UX excellence and performance.',
      technologies: ['React', 'Node.js', 'MongoDB', 'Firebase'],
      image: '/fashion-marketplace-platform-design.jpg',
      link: '#',
      github: '#',
      featured: true,
      duration: '2023',
    ...
    },
    {
      ...
      image: '/analytics-dashboard-lead-generation.jpg',
      link: '#',
      github: '#',
      featured: true,
      duration: '2023',
    ...
    },
    {
      ...
      image: '/tech-consulting-services-website.jpg',
      link: '#',
      github: '#',
      featured: true,
      duration: '2024',
  ...
  return (
    <section ref={containerRef} id="projects" className="py-32 px-4 md:px-8 relative overflow-hidden bg-gradient-to-b from-background to-background/95">
      <div className="absolute inset-0 -z-10">
        <SectionMeshBg />
        <div className="absolute top-40 right-20 w-96 h-96 bg-primary/10 rounded-full blur-3xl opacity-15 animate-blob"></div>
        <div className="absolute bottom-32 left-10 w-80 h-80 bg-accent/10 rounded-full blur-3xl opacity-15 animate-blob" style={{ animationDelay: '4s' }}></div>
      </div>
      
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="animate-on-scroll mb-24">
          <h2 className="text-5xl md:text-6xl font-black mb-6 text-balance">Featured Work</h2>
          <p className="text-lg text-muted-foreground max-w-2xl">Curated projects demonstrating expertise in frontend development and UX design.</p>
        </div>
            ...
            >
              <div className={`grid md:grid-cols-2 gap-12 items-center ${index % 2 === 1 ? 'md:[direction:rtl]' : ''}`}>
                <div className={`${index % 2 === 1 ? 'md:order-2' : ''}`}>
                  <div className={`relative overflow-hidden rounded-xl bg-gradient-to-br ${project.color} border border-border/50 aspect-video group-hover:border-primary/50 transition-all`}>
                    <img
                ...
                <div className={`${index % 2 === 1 ? 'md:order-1' : ''} space-y-6`}>
                  <div>
                    <p className="text-primary/70 text-sm font-semibold tracking-wide uppercase">{project.duration}</p>
                    <h3 className="text-4xl md:text-5xl font-black mt-2">{project.title}</h3>
                  </div>
                  ...
                  <div className="flex gap-4 pt-6">
                    ...
                    >
                      <span className="relative z-10">View Project</span> <ArrowUpRight size={16} className="group-hover/btn:translate-x-1 group-hover/btn:-translate-y-1 transition-transform" />
                    </a>
                    <a
                      href={project.github}
              ...
              >
                ...
                <div className="flex flex-wrap gap-2">
                  {project.technologies.map((tech) => (
                    <span
                      key={tech}
                      className="px-2 py-1 bg-primary/10 text-primary text-xs rounded font-medium border border-primary/30 hover:border-primary/50 transition-all"
                    >
                      {tech}
                    </span>
                  ))}
